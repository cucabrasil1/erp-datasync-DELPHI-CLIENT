...
type
  TEntityVendedor = class(TEntityBase)
  protected
...
class function TEntityVendedor.GetTableNameClass: string;
begin
  Result := 'C000008';
end;

function TEntityVendedor.GetResourceName: string;
begin
  Result := '/vendedor';
end;

function TEntityVendedor.GetRecord(ACodRecord: string): TDataSet;
var
  Qry: TFDQuery;
begin
...
function TEntityVendedor.GetApiId(ADataSet: TDataSet): string;
begin
  if FDatabaseType = dtIndustrial then
    Result := ADataSet.FieldByName('IDVENDEDOR').AsString
  else
    Result := ADataSet.FieldByName('IDVENDEDOR').AsString;
end;

function TEntityVendedor.MapToJson(ADataSet: TDataSet): TJSONObject;
begin
...
function TEntityVendedor.GetUnsyncedRecords: TDataSet;
var
  Qry: TFDQuery;
begin
  Qry := TFDQuery.Create(nil);
  try
    Qry.Connection := FConnection;
    Qry.SQL.Add('select * from ' + GetTableNameClass);
    Qry.SQL.Add('where IDVENDEDOR is null');
...
function TEntityVendedor.GetBatchSize: Integer;
begin
  Result := 1000;
end;

function TEntityVendedor.GetSyncAllMessage(ASuccess: Boolean; const AErrorMsg: string): string;
begin
  if ASuccess then
    Result := 'Vendedores sincronizados com sucesso'
  else
    Result := 'Erro em Vendedores: ' + AErrorMsg;
end;

procedure TEntityVendedor.StoreApiIdBack(ACodRecord, AApiId: string);
var
  Qry: TFDQuery;
begin
  Qry := TFDQuery.Create(nil);
  try
    Qry.Connection := FConnection;
    Qry.SQL.Add('update ' + GetTableNameClass + ' set IDVENDEDOR = :apiid');
    Qry.SQL.Add('where codigo = :cod');
...
initialization
  TEntityFactory.RegisterEntity(TEntityVendedor.GetTableNameClass, TEntityVendedor);
end.
