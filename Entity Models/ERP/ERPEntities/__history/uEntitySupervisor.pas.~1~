...
type
  TEntitySupervisor = class(TEntityBase)
  protected
...
class function TEntitySupervisor.GetTableNameClass: string;
begin
  Result := 'C000008';
end;

function TEntitySupervisor.GetResourceName: string;
begin
  Result := '/supervisor';
end;

function TEntitySupervisor.GetRecord(ACodRecord: string): TDataSet;
var
  Qry: TFDQuery;
begin
...
function TEntitySupervisor.GetApiId(ADataSet: TDataSet): string;
begin
  if FDatabaseType = dtIndustrial then
    Result := ADataSet.FieldByName('IDSUPERVISOR').AsString
  else
    Result := ADataSet.FieldByName('IDSUPERVISOR').AsString;
end;

function TEntitySupervisor.MapToJson(ADataSet: TDataSet): TJSONObject;
begin
...
function TEntitySupervisor.GetUnsyncedRecords: TDataSet;
var
  Qry: TFDQuery;
begin
  Qry := TFDQuery.Create(nil);
  try
    Qry.Connection := FConnection;
    Qry.SQL.Add('select * from ' + GetTableNameClass);
    Qry.SQL.Add('where IDSUPERVISOR is null');
...
function TEntitySupervisor.GetBatchSize: Integer;
begin
  Result := 1000;
end;

function TEntitySupervisor.GetSyncAllMessage(ASuccess: Boolean; const AErrorMsg: string): string;
begin
  if ASuccess then
    Result := 'Supervisores sincronizados com sucesso'
  else
    Result := 'Erro em Supervisores: ' + AErrorMsg;
end;

procedure TEntitySupervisor.StoreApiIdBack(ACodRecord, AApiId: string);
var
  Qry: TFDQuery;
begin
  Qry := TFDQuery.Create(nil);
  try
    Qry.Connection := FConnection;
    Qry.SQL.Add('update ' + GetTableNameClass + ' set IDSUPERVISOR = :apiid');
    Qry.SQL.Add('where codigo = :cod');
...
initialization
  TEntityFactory.RegisterEntity(TEntitySupervisor.GetTableNameClass, TEntitySupervisor);
end.
